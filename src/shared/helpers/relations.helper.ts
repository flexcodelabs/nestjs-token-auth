...
import { EntityMetadata, EntitySchema, ObjectType } from 'typeorm';
import { EmbeddedMetadata } from 'typeorm/metadata/EmbeddedMetadata';
import { RelationMetadata } from 'typeorm/metadata/RelationMetadata';
import { RelationParams } from '../interfaces/relations.interface';

export const extractGraphRelations = (params: RelationParams) => {
  return new RelationMapper()?.buildRelationListForQuery(
    params.entity,
    params.info,
  );
};
...
export class RelationMapper {
  ...
  public buildRelationListForQuery(
    ...
    const rootNode = info.fieldNodes.find(
      (fieldNode) => fieldNode.name.value === info.fieldName,
    );

    if (rootNode == null) {
      throw new Error(
        `Could not locate field named "${info.fieldName}" in query info"`,
      );
    }
  ...
  public buildRelationList(
    ...
    selectionSet.selections.forEach((selectionNode: SelectionNode) => {
      const currentPropertyPath: string[] = (basePropertyPath ?? '')
        .split('.')
        .filter((path) => path !== '');
      ...
      if (nodeName != null) {
        ...
        const propMetadata =
          this.getEntityMetadata(
            currentTargetEntity,
          ).findRelationWithPropertyPath(propPath) ||
          this.getEntityMetadata(
            currentTargetEntity,
          ).findEmbeddedWithPropertyPath(propPath);
      ...
      nestedRelations.forEach((nestedRelation) =>
        relationNames.add(nestedRelation),
      );
    });
  ...
  public findQueryNode(
    ...
    const baseNode = info.fieldNodes.find(
      (fieldNode) => fieldNode.name.value === info.fieldName,
    );
    ...
    fieldPath.split('.').forEach((fieldName): void => {
      const selectionSet = this.getSelectionSetFromNode(
        currentBaseNode,
        info.fragments,
      );
      ...
      const foundNode = this.findFieldInSelection(
        fieldName,
        selectionSet,
        info.fragments,
      );
  ...
  private getEntityMetadata(entity): EntityMetadata {
    return entity.getRepository().metadata;
  }
    ...
    switch (selectionNode.kind) {
      case 'FragmentSpread':
        if (fragments == null || fragments[selectionNode.name.value] == null) {
          throw new Error(
            `Could not find the fragment named ${selectionNode.name.value} referenced in the query.`,
          );
        }
  ...
  private findFieldInSelection(
    ...
    for (const selectionNode of selectionSet.selections) {
      ...
      if (
        ...
        foundNode = this.findFieldInSelection(
          fieldName,
          selectionSet,
          fragments,
        );
